...
namespace Pomelo.DotNetClient
{
    ...
    {
        private const string xxteakey = "7AEC4MA152BQE9HWQ7KB";
        ...
        public MessageProtocol(JsonObject dict, JsonObject serverProtos, JsonObject clientProtos,bool isCrypto)
        {
        ...
        public byte[] encode(string route, uint id, JsonObject msg)
        {
            ...
            else
            {
                ...
                if (isCrypto)
                {
                    string encrypt = XXTEA.EncryptToBase64String(msg.ToString(), xxteakey);
                    body = Encoding.UTF8.GetBytes(encrypt);
                }
                else
                {
        ...
        {
            // Debug.Log($"Message length={buffer.Length}");
            //Decode head
            //Get flag
            byte flag = buffer[0];
            ...
            else
            {
                ...
                // Log.Info($"DecryptToString rawString={rawString}");
                
                // 解密
                if (isCrypto)
                {
                    // string base64Str = Encoding.UTF8.GetString(body);
                    // Log.Info($"{base64Str.Length} | "+base64Str);
                    // byte[] fromBase64String = Convert.FromBase64String(base64Str);
                    // rawString = XXTEA.DecryptToString(fromBase64String,xxteakey);
                    //
                    // Log.Info($"DecryptToString rawString={rawString}");

                    rawString = XXTEA.DecryptBase64StringToString(rawString, xxteakey);
                }
